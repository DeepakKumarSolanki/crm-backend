...
const {
  initializeLeadSchema
} = require("../model/schema/lead");
const {
  initializeContactSchema
} = require("../model/schema/contact");
const {
  initializePropertySchema
...
const {
  contactFields
} = require('./contactFields.js');
const {
  leadFields
} = require('./leadFields.js');
...
const connectDB = async (DATABASE_URL, DATABASE) => {
  try {
    ...
    /*  */
    await initializedSchemas();
    let adminExisting = await User.find({
      role: 'superAdmin'
    });
    if (adminExisting.length <= 0) {
      ...
      await User.findByIdAndUpdate(adminExisting[0]._id, {
        username: 'admin@kingsmenrealty.com'
      });
      console.log("Admin Update successfully..");
    }
