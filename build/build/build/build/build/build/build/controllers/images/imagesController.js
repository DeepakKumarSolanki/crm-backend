...
const addAuthImg = async (req, res) => {
  try {
    ...
    return res.status(400).json({
      success: false,
      message: 'Failed to add auth image'
    });
  }
};
const addAuthAndLogoImg = async (req, res) => {
  try {
    if (!req.files?.authImg && !req?.files?.logoLgImg && !req?.files?.logoSmImg) {
      ...
      return res.status(400).send({
        success: false,
        message: 'No logo files uploaded'
      });
    }
    const {
      logoLgImg,
...
const UpdateAuthImg = async (req, res) => {
  try {
    ...
    const existingDocument = await Img.findOne({
      _id: id
    });
    if (existingDocument) {
      const url = req.protocol + '://' + req.get('host');
    ...
    return res.status(400).json({
      success: false,
      message: 'Failed to update auth Image'
    });
  }
};
const changeLogoImg = async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      success: false,
      message: 'Failed to set logo imges'
    });
  }
};
...
const setActiveImg = async (req, res) => {
  try {
    ...
    const imageData = await Img.findByIdAndUpdate(req.params.id, {
      isActive: isActive
    }, {
      new: true
    });
