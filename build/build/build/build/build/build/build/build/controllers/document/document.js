...
const file = async (req, res) => {
  try {
    ...
    let folder = await DocumentSchema.findOne({
      folderName
    });
    if (!folder) {
      ...
      folder = new DocumentSchema({
        folderName,
        file: files,
        // Directly assign the files array
        createBy
      });
    } else {
    ...
    await folder.save();
    res.json({
      message: 'Folder and files added successfully'
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({
...
const downloadFile = async (req, res) => {
  try {
    ...
    // Check if the folder exists in the database
    const folder = await DocumentSchema.findOne({
      'file._id': id
    });
    if (!folder) {
      return res.status(404).json({
...
const LinkDocument = async (req, res) => {
  try {
    ...
    if (!linkContact && !linkLead) {
      return res.status(404).json({
        message: 'Select valid contact or lead '
      });
    }
    ...
    res.status(200).json({
      message: "File link successfully.",
      document: savedFolder
    });
  } catch (err) {
    res.status(500).json({
      message: "Error Link file.",
      error: err
    });
  }
};
