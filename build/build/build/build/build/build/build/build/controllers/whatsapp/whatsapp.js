...
const addMessage = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      message: 'WhatsApp message added successfully',
      data: newMessage
    });
  } catch (error) {
...
const getAllMessages = async (req, res) => {
  try {
    const messages = await WhatsApp.find().populate('createBy', 'firstName lastName').populate('sender').populate('recipient').select('sender recipient recipientNumber message startDate createBy createByLead'); // Select necessary fields
...
const deleteManyMessages = async (req, res) => {
  try {
    ...
    const deletedMessages = await WhatsApp.deleteMany({
      _id: {
        $in: ids
      }
    });
