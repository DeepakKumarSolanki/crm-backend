    ...
    if (collectionExists) {
      ...
      if (existingSchema) {
        ...
        if (updateResult?.modifiedCount > 0) {
          existingSchema?.add({
            [fieldName]: fieldType || 'Mixed'
          }); // Add fields in existingSchema
      ...
      if (existingSchema) {
        ...
        if (updateResult?.matchedCount > 0 && updateResult?.modifiedCount > 0) {
          ...
          if (isFieldNameChanged) {
            ...
            existingSchema.add({
              [updatedFieldName]: updatedFieldBcType
            });
          }
          return res.status(200).json({
            success: true,
...
const view = async (req, res) => {
  try {
    const customFieldDoc = await CustomField.findOne({
      _id: req.params.id
    });
    if (!customFieldDoc) return res.status(404).json({
      success: false,
...
const deleteField = async (req, res) => {
  try {
    ...
    const customFieldBeforePull = await CustomField.findOne({
      _id: moduleId
    });
...
const createNewModule = async (req, res) => {
  try {
    ...
    if (existingModule) {
      return res.status(400).json({
        success: false,
        message: `Module name not available !`
      });
    }
...
const changeIcon = async (req, res) => {
  try {
    ...
    const existingDocument = await CustomField.findOne({
      _id: id
    });
    if (existingDocument) {
...
const changeModuleName = async (req, res) => {
  try {
    const moduleName = req.body.moduleName;
    const oldModule = await CustomField.findOne({
      _id: req.params.id
    });
    ...
    await RoleAccess.updateMany({
      "access.title": oldModule.moduleName
    },
    // Filter criteria to find the documents
    {
      $set: {
        "access.$.title": moduleName
      }
    } // Update operation
    );
...
const editSingleHeading = async (req, res) => {
  try {
    ...
    let result = await CustomField.updateOne({
      ...
      $set: {
        'headings.$': req.body.updatedHeading
      }
    });
    if (result?.modifiedCount > 0) {
      return res.status(200).json({
...
const deleteManyHeadings = async (req, res) => {
  try {
    ...
    if (result?.modifiedCount > 0) {
      return res.status(200).json({
        message: "Headings removed successfully",
        result
      });
    } else {
      return res.status(404).json({
...
const changeIsTableField = async (req, res) => {
  try {
    ...
    let result = await CustomField.findOneAndUpdate({
      ...
      $set: {
        'fields.$.isTableField': isTableField
      }
    }, {
      new: true
    });
    ...
    const successMessage = isTableField ? 'Field will now be included in the table.' : 'Field will no longer be included in the table.';
    return res.status(200).json({
      success: true,
  ...
  try {
    ...
    const updateOperations = updates.map(({
      ...
      updateOne: {
        ...
        update: {
          $set: {
            'fields.$[field].isTableField': isTableField
          }
        },
        arrayFilters: [{
          'field._id': new mongoose.Types.ObjectId(fieldId)
        }]
      }
    }));
...
const deletmodule = async (req, res) => {
  try {
    ...
    await RoleAccess.updateMany({}, {
      $pull: {
        access: {
          "title": accessName.moduleName
        } // remove the access object with matching title
      }
    });
    res.status(200).json({
...
const deleteManyModule = async (req, res) => {
  try {
    const module = await CustomField.deleteMany({
      _id: {
        $in: req.body
      }
    }, {
    ...
    res.status(200).json({
      message: "Many module delete successfully",
      module
    });
  } catch (err) {
    res.status(404).json({
...
const changeFieldsBelongsTo = async (req, res) => {
  try {
    ...
    if (!headingExists) {
      return res.status(404).json({
        success: false,
        message: "Module or Heading not found"
      });
    }
    const updateOperations = updates?.map(update => ({
      updateOne: {
        ...
        update: {
          $set: {
            'fields.$[field].belongsTo': update.belongsTo
          }
        },
        arrayFilters: [{
