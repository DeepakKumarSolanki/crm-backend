...
const add = async (req, res) => {
  try {
    ...
    if (existingValidation) {
      return res.status(400).json({
        success: false,
        message: `Validation name already exist`
      });
    }
...
const view = async (req, res) => {
  try {
    ...
    if (!validationDoc) return res.status(404).json({
      success: false,
...
const deleteValidationDocument = async (req, res) => {
  try {
    ...
    if (result?.modifiedCount > 0) {
      return res.status(200).json({
        message: "Validdation removed successfully",
        result
      });
    } else {
      return res.status(404).json({
...
const deleteManyValidationDocuments = async (req, res) => {
  try {
    ...
    if (result?.modifiedCount > 0) {
      return res.status(200).json({
        message: "Validations removed successfully",
        result
      });
    } else {
      return res.status(404).json({
