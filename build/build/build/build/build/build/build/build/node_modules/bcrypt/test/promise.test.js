...
test('hash', () => {
  return expect(bcrypt.genSalt(10).then(salt => bcrypt.hash('password', salt))).resolves.toBeDefined();
});
test('hash_rounds', () => {
...
test('hash_empty_strings', () => {
  expect.assertions(2);
  return Promise.all([expect(bcrypt.genSalt(10).then(salt => bcrypt.hash('', salt))).resolves.toBeDefined(), expect(bcrypt.hash('', '')).rejects.toThrow('')]);
});
test('hash_no_params', () => {
  expect.assertions(1);
...
test('hash_compare', () => {
  ...
  }).then(hash => Promise.all([expect(bcrypt.compare("test", hash)).resolves.toEqual(true), expect(bcrypt.compare("blah", hash)).resolves.toEqual(false)]));
});
