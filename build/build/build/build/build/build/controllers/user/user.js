  ...
  try {
    ...
    if (user) {
      return res.status(400).json({
        message: "Admin already exist please try another email"
      });
    } else {
      // Hash the password
  ...
  try {
    ...
    if (user) {
      ...
      const newUser = new User({
        ...
        phoneNumber,
        assignedManager,
        // Changed field name from employeeId
        designation,
        // Changed field name from employeeRole
...
let deleteData = async (req, res) => {
  try {
    ...
    if (process.env.DEFAULT_USERS.includes(user.username)) {
      return res.status(400).json({
        message: `You don't have access to delete ${user.username}`
      });
    }
