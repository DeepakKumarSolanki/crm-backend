...
const {
  Property
} = require('../../model/schema/property');
const {
  whatsApp
} = require('../../model/schema/whatsApp');
...
const lineChart = async (req, res) => {
  ...
  let result = [{
    ...
    name: "Meetings",
    length: meetingHistoryData?.length,
    color: "purple"
  }, {
    name: "Emails",
  ...
  try {
    ...
    let matchFilter = {
      timestamp: {
        $gte: startDate,
        $lte: endDate
      } // Filter documents with timestamp between startDate and endDate
    };
              ...
              date: {
                $dateToString: {
                  date: {
                    $dateFromParts: {
                      year: {
                        $ifNull: ['$$email._id.year', '$_id.year']
                      },
                      month: {
                        $ifNull: ['$$email._id.month', 1]
                      },
                      day: {
                        $ifNull: ['$$email._id.day', 1]
                      }
                    }
                  }
                }
              ...
              date: {
                $dateToString: {
                  date: {
                    $dateFromParts: {
                      year: {
                        $ifNull: ["$$call._id.year", "$_id.year"]
                      },
                      month: {
                        $ifNull: ["$$call._id.month", 1]
                      },
                      day: {
                        $ifNull: ["$$call._id.day", 1]
                      }
                    }
                  }
                }
    ...
    // Unwind the "Calls" array to work on each element separately
    {
      $sort: {
        "Calls.date": 1
      }
    },
    // Sort the Calls array by the "date" field in ascending order
    {
      $group: {
        _id: "$_id",
        startDate: {
              ...
              date: {
                $dateToString: {
                  date: {
                    $dateFromParts: {
                      year: {
                        $ifNull: ["$$msg._id.year", "$_id.year"]
                      },
                      month: {
                        $ifNull: ["$$msg._id.month", 1]
                      },
                      day: {
                        $ifNull: ["$$msg._id.day", 1]
                      }
                    }
                  }
                }
    ...
    // Unwind the "TextMsges" array to work on each element separately
    {
      $sort: {
        "TextMsges.date": 1
      }
    },
    // Sort the TextMsges array by the "date" field in ascending order
    {
      $group: {
        _id: "$_id",
        startDate: {
