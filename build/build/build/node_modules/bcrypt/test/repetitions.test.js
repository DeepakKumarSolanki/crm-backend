...
test('salt_length', () => {
  ...
  return Promise.all(Array.from({
    length: EXPECTED
  }, () => bcrypt.genSalt(10).then(salt => expect(salt).toHaveLength(29))));
});
test('test_hash_length', () => {
  ...
  return Promise.all(Array.from({
    length: EXPECTED
  }, () => bcrypt.hash('test', SALT).then(hash => expect(hash).toHaveLength(60))));
});
test('test_compare', () => {
  expect.assertions(EXPECTED);
  ...
  return Promise.all(Array.from({
    length: EXPECTED
  }, () => bcrypt.compare('test', HASH).then(match => expect(match).toEqual(true))));
});
