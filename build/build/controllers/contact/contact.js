...
const index = async (req, res) => {
  ...
  try {
    const query = {
      ...req.query,
      deleted: false
    }; // Ensure `deleted` is part of the query
    ...
    let allData = await Contact.find(query).populate({
      ...
      match: {
        deleted: false
      } // Populate only if createBy.deleted is false
    }).populate({
      path: 'assignedTo',
      select: 'email',
      // Include only the `email` field for assignedTo
      match: {
        deleted: false
      } // Populate only if assignedTo.deleted is false
    }).exec();
  ...
  try {
    ...
    if (!leadStatus) {
      return res.status(400).json({
        success: false,
        message: "Invalid leadRemark for status update"
      });
    }
...
const changeStatus = async (req, res) => {
  try {
    ...
    if (!leadStatus) {
      return res.status(400).json({
        success: false,
        message: "Invalid name for status update"
      });
    }
...
const view = async (req, res) => {
  try {
    ...
    let EmailHistory = await email.aggregate([{
      $match: {
        createByContact: contact._id
      }
    }, {
      $lookup: {
        from: 'Contacts',
        // Assuming this is the collection name for 'contacts'
      ...
      $addFields: {
        ...
        createByName: {
          $cond: {
            if: '$createByRef',
            then: {
              $concat: ['$createByRef.title', ' ', '$createByRef.firstName', ' ', '$createByRef.lastName']
            },
            else: {
              $concat: ['$createByrefLead.leadName']
            }
          }
        }
      }
    ...
    const Document = await DocumentSchema.aggregate([{
      ...
      $match: {
        'file.deleted': false,
        'file.linkContact': contact._id
      }
    }, {
      $lookup: {
        from: 'User',
        // Replace 'users' with the actual name of your users collection
        localField: 'createBy',
        foreignField: '_id',
        // Assuming the 'createBy' field in DocumentSchema corresponds to '_id' in the 'users' collection
        as: 'creatorInfo'
      }
    }, {
      $unwind: {
      ...
      $match: {
        'creatorInfo.deleted': false
      }
    }, {
      $group: {
        _id: '$_id',
        // Group by the document _id (folder's _id)
        folderName: {
          $first: '$folderName'
        },
        // Get the folderName (assuming it's the same for all files in the folder)
        createByName: {
          $first: {
        ...
        files: {
          $push: '$file'
        } // Push the matching files back into an array
      }
    }, {
      $project: {
        creatorInfo: 0
